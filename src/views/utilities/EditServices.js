...
const ResultsTable = () => {
  ...
  const { dbUrl } = useContext(UsernameContext);
  ...
  const handleSendFile = async (e) => {
    ...
    try {
      const res = await axios.post(`${dbUrl}/img/upload`, {
        image_url: preview
      });
  ...
  const handleEditSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`${dbUrl}/api/storeEditedData`, {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      try {
        const response = await fetch(`${dbUrl}/api/manageservices`);
  ...
  const handleEditModal = async (id) => {
    ...
    try {
      const response = await axios.get(`${dbUrl}/api/fetchServiceForEdit/${id}`);
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.put(`${dbUrl}/api/deleteService/${id}`);
