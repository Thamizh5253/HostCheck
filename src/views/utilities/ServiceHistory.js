...
const ResultsTable = () => {
  const { username, dbUrl } = useContext(UsernameContext);
  ...
  useEffect(() => {
    const fetchResults = async () => {
      try {
        const response = await fetch(`${dbUrl}/api/servicehistory/${username}`);
    ...
    fetchResults();
  }, [username]); // Update results when username changes
                ...
                {statusFilteredResults.map((result, index) => (
                  <TableRow key={result._id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                    ...
                    <TableCell align="center">
                      {result.status === 0 && (
                        <Button
                          style={{ color: 'red', border: '1px solid red', padding: '5px 42px', borderRadius: '4px', cursor: 'pointer' }}
                        >
                          Pending
                        </Button>
                      )}
