...
const ProfileSection = () => {
  ...
  const { username, setProfile, profile, setAuth, dbUrl } = useContext(UsernameContext);
  ...
  const handleEditModal = async () => {
    ...
    try {
      const response = await axios.get(`${dbUrl}/api/fetchEditAccountInfo/${username}`);
  ...
  const handleEditSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`${dbUrl}/api/storeEditedUserData`, {
        method: 'POST',
        headers: {
  ...
  const handleEditSubmitPassword = async (event) => {
    ...
    try {
      const response = await fetch(`${dbUrl}/api/changepassword`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  useEffect(() => {
    const fetchUsername = async () => {
      try {
        const response = await fetch(`${dbUrl}/api/fetchusername/${username}`);
        if (!response.ok) {
